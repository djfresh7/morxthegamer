...
ERROR_TEXT = "There was an error running this command."

with open("botsfiles/cmds.txt", "r") as cmds:
  COMMANDS_TEXT = cmds.read()
with open("botsfiles/shorts.txt", "r") as shrts:
  SHORTS_TEXT = shrts.read()
...
def url_shorts(message):
  bot.reply_to(message, SHORTS_TEXT)
...
def randnum(message):
  n1 = int(message.text[9:-3])
  n2 = int(message.text[-3:])
  num = random.randrange(n1, n2)
  try:
...
def memevid(message):
  download_file(VIDEO_LINK, "botsfiles/mvd.gif")
...
def toastvid(message):
  download_file(VIDEO_LINK_2, "botsfiles/tvd.gif")
...
def rickroll(message):
  download_file(VIDEO_LINK_3, "botsfiles/rkr.gif")
...
def ktype(message):
  download_file(AUDIO_LINK_2, "botsfiles/ktype.mp3")
...
def burp(message):
  download_file(AUDIO_LINK, "botsfiles/burp.mp3")
